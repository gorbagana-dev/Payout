...
class SPLToken2022Creator {
  ...
  async createTokenWithMetadata(metadata: TokenMetadata): Promise<string> {
    console.log("\n🚀 Starting SPL Token-2022 creation with metadata...");

    try {
      // Calculate the required space for the mint account with metadata pointer extension
      ...
      console.log("📝 Transaction created with mint and metadata pointer");
      ...
      // Now initialize the metadata using the tokenMetadataInitialize function
...
// Main execution
async function main() {
  console.log("🌟 SPL Token-2022 Creator with Metadata");
  console.log("=====================================\n");

  try {
    const creator = new SPLToken2022Creator();
    ...
    const mintAddress = await creator.createTokenWithMetadata(tokenMetadata);
...
export { SPLToken2022Creator, TokenMetadata };
