...
// COMMENTED OUT: Unused unlock schedules - retained for future use
...
// COMMENTED OUT: Unused function - retained for future use
// fn add_stakes(
...
// Initial allocation for the Lex treasury wallet - secures chain stability and prevents potential exploits
const MAIN_WALLET_PUBKEY: &str = "9Edy2fgfvhSJuUpYwaMaBfTyyQN6QDUo8W46igTYEJTe";
const MAIN_WALLET_LAMPORTS: u64 = 268_313_096 * LAMPORTS_PER_SOL; // 268.313096 million SOL (1000 million total - 500 million bootstrap - 231.686904 million)

/// Add accounts that should be present in genesis; skip for development clusters
pub fn add_genesis_accounts(genesis_config: &mut GenesisConfig, _issued_lamports: u64) {
    if genesis_config.cluster_type == ClusterType::Development {
        return;
    }

    // Add initial allocation for the main wallet (689 million SOL)
    let main_wallet_pubkey = Pubkey::from_str(MAIN_WALLET_PUBKEY)
        .expect("Invalid main wallet pubkey");
    genesis_config.accounts.insert(
        main_wallet_pubkey,
        Account {
            lamports: MAIN_WALLET_LAMPORTS,
            data: vec![],
    ...
    // COMMENTED OUT: All stake allocations commented out for fresh chain creation
    ...
    // COMMENTED OUT: Community pool stake commented out for fresh chain creation
