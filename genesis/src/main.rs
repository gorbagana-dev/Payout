...
/// Load airdrop accounts from a CSV file.
/// CSV format: address,lamports
/// Example:
///   address,lamports
///   7xKXtg2CW87d97TXJSDpbD5jBkheTqA83TZRuJosgAsU,1000000000
///   9aE476sH92Vz7DMPyq5WLPkrKWivxeuTKEFKd2sZZcde,2000000000
pub fn load_airdrop_accounts(file: &str, genesis_config: &mut GenesisConfig) -> io::Result<u64> {
    use io::BufRead;
    
    let mut lamports = 0;
    let accounts_file = File::open(file)?;
    let reader = io::BufReader::new(accounts_file);
    let mut line_num = 0;
    ...
    for line in reader.lines() {
        ...
        // Parse CSV: address,lamports
        let parts: Vec<&str> = line.split(',').map(|s| s.trim()).collect();
        if parts.len() < 2 {
            return Err(io::Error::other(format!(
                "Invalid CSV format at line {line_num}: expected 'address,lamports', got '{line}'"
            )));
        }
        ...
        // Skip header row
        if address_str.eq_ignore_ascii_case("address") || address_str.eq_ignore_ascii_case("pubkey") {
            continue;
        }
        ...
        let pubkey = Pubkey::from_str(address_str).map_err(|err| {
            io::Error::other(format!(
                "Invalid address at line {line_num}: '{address_str}': {err}"
            ))
        })?;
        
        // Parse lamports amount
        let account_lamports: u64 = amount_str.parse().map_err(|err| {
            io::Error::other(format!(
                "Invalid lamports amount at line {line_num}: '{amount_str}': {err}"
            ))
        })?;
        
        // Create native SOL account (owner = system program)
        let account = AccountSharedData::new(account_lamports, 0, &system_program::id());
        lamports += account_lamports;
        genesis_config.add_account(pubkey, account);
        accounts_added += 1;
    }

    println!("Loaded {accounts_added} airdrop accounts from {file}");
    Ok(lamports)
}
...
fn main() -> Result<(), Box<dyn error::Error>> {
    ...
    let matches = App::new(crate_name!())
        ...
        .arg(
            Arg::with_name("airdrop_accounts_file")
                .long("airdrop-accounts-file")
                .value_name("FILENAME")
                .takes_value(true)
                .multiple(true)
                .help("CSV file with airdrop accounts. Format: address,lamports (one per line, header optional)"),
        )
        .arg(
    ...
    if let Some(files) = matches.values_of("airdrop_accounts_file") {
        for file in files {
            load_airdrop_accounts(file, &mut genesis_config)?;
        }
    }
...
\"[103,27,132,107,42,149,72,113,24,138,225,109,209,31,158,6,26,11,8,76,24,128,131,215,156,80,251,114,103,220,111,235,56,22,87,5,209,56,53,12,224,170,10,66,82,42,11,138,51,76,120,27,166,200,237,16,200,31,23,5,57,22,131,221]\":
  ...
  executable: true";
    ...
    #[test]
    fn test_load_airdrop_accounts_from_csv() {
        // Test invalid file returns error
        assert!(load_airdrop_accounts("unknownfile", &mut GenesisConfig::default()).is_err());

        // Test valid CSV with header, comments, and empty lines
        let mut genesis_config = GenesisConfig::default();
        ...
        let path = Path::new("test_airdrop_accounts.csv");
        let mut file = File::create(path).unwrap();
        file.write_all(csv_content.as_bytes()).unwrap();

        let total_lamports = load_airdrop_accounts("test_airdrop_accounts.csv", &mut genesis_config)
            .expect("Failed to load airdrop accounts");

        remove_file(path).unwrap();

        // Verify total lamports
        assert_eq!(total_lamports, 6000000000);

        // Verify all accounts were added
        assert_eq!(genesis_config.accounts.len(), 3);

        // Verify account balances
        assert_eq!(genesis_config.accounts[&pubkey1].lamports, 1000000000);
        assert_eq!(genesis_config.accounts[&pubkey2].lamports, 2000000000);
        assert_eq!(genesis_config.accounts[&pubkey3].lamports, 3000000000);

        // Verify all accounts are owned by system program
        assert_eq!(genesis_config.accounts[&pubkey1].owner, system_program::id());
        assert_eq!(genesis_config.accounts[&pubkey2].owner, system_program::id());
        assert_eq!(genesis_config.accounts[&pubkey3].owner, system_program::id());

        // Verify accounts are not executable
        assert!(!genesis_config.accounts[&pubkey1].executable);
        assert!(!genesis_config.accounts[&pubkey2].executable);
        assert!(!genesis_config.accounts[&pubkey3].executable);

        // Test more accounts can be appended
        let pubkey4 = solana_pubkey::new_rand();
        let csv_content2 = format!("{},5000000000\n", pubkey4);

        let path2 = Path::new("test_airdrop_accounts2.csv");
        let mut file2 = File::create(path2).unwrap();
        file2.write_all(csv_content2.as_bytes()).unwrap();

        load_airdrop_accounts("test_airdrop_accounts2.csv", &mut genesis_config)
            .expect("Failed to load second airdrop file");

        remove_file(path2).unwrap();

        // Verify new account was appended
        assert_eq!(genesis_config.accounts.len(), 4);
        assert_eq!(genesis_config.accounts[&pubkey4].lamports, 5000000000);

        // Test invalid address returns error
        let path3 = Path::new("test_airdrop_invalid.csv");
        let mut file3 = File::create(path3).unwrap();
        file3.write_all(b"invalid_address,1000\n").unwrap();

        let result = load_airdrop_accounts("test_airdrop_invalid.csv", &mut GenesisConfig::default());
        remove_file(path3).unwrap();
        assert!(result.is_err());

        // Test invalid amount returns error
        let path4 = Path::new("test_airdrop_invalid_amt.csv");
        let mut file4 = File::create(path4).unwrap();
        file4.write_all(format!("{},not_a_number\n", pubkey1).as_bytes()).unwrap();

        let result = load_airdrop_accounts("test_airdrop_invalid_amt.csv", &mut GenesisConfig::default());
        remove_file(path4).unwrap();
        assert!(result.is_err());
    }
}
