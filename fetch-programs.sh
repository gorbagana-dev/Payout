...
fetch_program() {
  ...
  # Set a safe cache directory if HOME is not properly set
  CACHE_DIR="${HOME:-/tmp}/.cache/solana-$prefix"

  if [[ -r "$CACHE_DIR/$so" ]]; then
    cp "$CACHE_DIR/$so" "$so"
  else
    ...
    # Only try to cache if we can create the directory
    if mkdir -p "$CACHE_DIR" 2>/dev/null; then
      cp "$so" "$CACHE_DIR/$so" 2>/dev/null || echo "Warning: Could not cache $so"
    else
      echo "Warning: Could not create cache directory $CACHE_DIR"
    fi
  fi
